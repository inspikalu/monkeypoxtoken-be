...
class SPLConfig extends token_1.default {
    ...
    async getTx(txId) {
        ...
        const getAndValidateTokenAccOwner = async (tokenAccAddress) => {
            ...
            if (!tokenAccInfoValue || Buffer.isBuffer(tokenAccInfoValue) || Buffer.isBuffer(tokenAccInfoValue.data))
    ...
    async _createTxUnsigned(amount, to, fee) {
        ...
        const fromTokenAccount = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, keys, mint, keys.publicKey);
        const toTokenAccount = await (0, spl_token_1.getAssociatedTokenAddress)(mint, new web3_js_1.PublicKey(to));
        transaction.add((0, spl_token_1.createTransferInstruction)(fromTokenAccount.address, toTokenAccount, keys.publicKey, +new bignumber_js_1.default(amount).toNumber()));
