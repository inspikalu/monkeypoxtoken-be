...
function createIrysUploader(context, uploaderOptions = {}) {
  ...
  const upload = async (files, options) => {
    ...
    const result = await PromisePool__default["default"].for(files).withConcurrency(uploaderOptions.uploadConcurrency ?? 10).onTaskFinished((_, pool) => options?.onProgress?.(pool.processedPercentage())).process(async file => {
